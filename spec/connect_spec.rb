...
module GustoPlaid
  class Connect
    describe 'get' do
      ...
      before do
        allow(GustoPlaid::RestClient).to receive(:post).and_return(response)
      end

      it 'converts gte to ISO8601 format' do
        expect(GustoPlaid::RestClient).to receive(:post).
            with('connect/get', hash_including(options: '{"gte":"2013-04-02"}'))
        GustoPlaid::Connect.get('access-token', gte: DateTime.new(2013, 04, 02, 12, 1, 0))
      end

      it 'converts gte from date to ISO8601 format' do
        expect(GustoPlaid::RestClient).to receive(:post).
            with('connect/get', hash_including(options: '{"gte":"2013-04-02"}'))
        GustoPlaid::Connect.get('access-token', gte: Date.new(2013, 04, 02))
      end

      it 'converts gte from time to ISO8601 format' do
        expect(GustoPlaid::RestClient).to receive(:post).
            with('connect/get', hash_including(options: '{"gte":"2013-04-02"}'))
        GustoPlaid::Connect.get('access-token', gte: Time.new(2013, 04, 02, 12, 1, 0))
      end
      ...
      it 'does not touch the options that is not date' do
        ...
        GustoPlaid::Connect.get('access-token', pending: true)
      end
      ...
      it 'does not convert to ISO8601 format from something it can not do' do
        ...
        GustoPlaid::Connect.get('access-token', gte: gte)
      end
    end
    ...
    describe 'base' do
      ...
      it 'converts gte to ISO8601 format' do
        ...
        GustoPlaid::Connect.base('1234', {username: 'username', password: 'password'}, gte: DateTime.new(2013, 04, 02, 12, 1, 0))
      end

      it 'converts gte from date to ISO8601 format' do
        expect(GustoPlaid::RestClient).to receive(:post).
            with('connect', hash_including(options: '{"gte":"2013-04-02"}'))
        GustoPlaid::Connect.base('1234', {username: 'username', password: 'password'}, gte: Date.new(2013, 04, 02))
      end

      it 'converts gte from time to ISO8601 format' do
        expect(GustoPlaid::RestClient).to receive(:post).
            with('connect', hash_including(options: '{"gte":"2013-04-02"}'))
        GustoPlaid::Connect.base('1234', {username: 'username', password: 'password'}, gte: Time.new(2013, 04, 02, 12, 1, 0))
      end
      ...
      it 'does not touch the options that is not date' do
        ...
        GustoPlaid::Connect.base('1234', {username: 'username', password: 'password'}, pending: true)
      end
      ...
      it 'does not convert to ISO8601 format from something it can not do' do
        ...
        GustoPlaid::Connect.base('1234', {username: 'username', password: 'password'}, gte: gte)
      end
    end
    ...
    describe 'step' do
      ...
      context 'when the first answer is given' do
        it 'responds with a status code of 201' do
          response = GustoPlaid::Connect.step('test', 'again', nil, 'bofa')
          expect(response[:code]).to eq(201)
        end
      end
      context 'when the final answer is given' do
        it 'responds with a status code of 200' do
          response = GustoPlaid::Connect.step('test', 'tomato', nil, 'bofa')
    ...
    describe 'delete' do
      it 'responds with a status code of 200' do
        response = GustoPlaid::Connect.delete('test_wells')
        expect(response[:code]).to eq(200)
      end
    end
  end
